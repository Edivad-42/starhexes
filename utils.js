...
const Utils = {
  ...
  makeAIMove: (aiUnits, playerUnits, config) => {
    if (aiUnits.length === 0) return null;

    const aiUnit = aiUnits[0];
    const allUnits = [...aiUnits, ...playerUnits];
    const validMoves = Utils.getValidMoves(aiUnit, allUnits, config);

    if (validMoves.length === 0) {
      return { unit: aiUnit, move: null };
    }
    ...
    return { unit: aiUnit, move: bestMove };
  },
